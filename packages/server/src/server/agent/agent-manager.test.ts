...
test("updateAgentMetadata updates live agent labels", async () => {
  ...
  expect(persisted?.title).toBe("Live title");
  expect(persisted?.labels).toEqual({ role: "worker" });
});
...
test("archiveSnapshot persists the latest snapshot as archived", async () => {
  const workdir = mkdtempSync(join(tmpdir(), "agent-manager-snapshot-archive-"));
  ...
  const archivedAt = "2026-05-20T00:00:00.000Z";
  const record = await manager.archiveSnapshot(snapshot.id, archivedAt);

  expect(record.archivedAt).toBe(archivedAt);
  expect((await storage.get(snapshot.id))?.archivedAt).toBe(archivedAt);
});
...
test("sendPromptToAgent handles archive-session as an out-of-band prompt", async () => {
...
test("syncNativeArchivedStateForStoredAgents archives only non-live stored records", async () => {
  ...
  class ArchivedDescriptorClient extends TestAgentClient {
    override async listImportableSessions() {
      return [
        {
          providerHandleId: "native-stored",
          cwd: workdir,
