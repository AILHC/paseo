...
export class AgentManager {
  ...
  async syncNativeArchivedStateForStoredAgents(options?: { cwd?: string }): Promise<void> {
    ...
    for (const [provider, providerRecords] of byProvider) {
      ...
      if (!client?.listImportableSessions) continue;
      let descriptors: ManagedImportableProviderSession[];
      try {
        descriptors = (
          await client.listImportableSessions({
            limit: 500,
          ...
          provider,
          providerHandleId: descriptor.providerHandleId,
          cwd: descriptor.cwd,
        ...
        this.logger.warn({ err: error, provider }, "Failed to list native importable sessions");
        continue;
      }
      const archivedByHandle = new Map<string, ManagedImportableProviderSession>();
      for (const descriptor of descriptors) {
        if (!descriptor.archivedAt) continue;
        archivedByHandle.set(descriptor.providerHandleId, descriptor);
      }
  ...
  async archiveSessionSnapshot(agentId: string, archivedAt: string): Promise<StoredAgentRecord> {
    return await this.archiveSnapshot(agentId, archivedAt);
  }
  ...
  async updateAgentMetadata(
    ...
    if (liveAgent) {
      ...
      if (updates.labels) {
        liveAgent.labels = { ...liveAgent.labels, ...updates.labels };
      }
      if (!normalizedTitle && !updates.labels) {
        return;
      }
